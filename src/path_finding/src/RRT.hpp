    ...
    if(mode == 0)
    {
        ...
        for(int i=0;i<obstacle_distance;++i) //look up obstacle around the robot situated at distance_detection from robot
        {
            for(int j=0;j<obstacle_distance;++j)
            {
                ...
                {
                    cv::Scalar h = 0;
                    h=(int) map.at<float>(yTemp + j,xTemp+i);
                    //if(h[0] < 254)
                    //return false;
                    if(h[0] >= 254)
                        return true;
    ...
    else
    {
        ...
        if(mode == 1 || mode == 4)
        {
            ...
            for (int j = 0; j < obstacle_distance ; ++j)
            {
                ...
                {
                    cv::Scalar k = (int) map.at<float>(yTemp + j,xTemp );
                    if(k[0] < 254)
                        return false;
                }
                if(is_in_map2(xTemp,yTemp,map.cols,1,j))
                {
                    cv::Scalar l =  (int) map.at<float>(yTemp + j ,xTemp+1 );
                    if(l[0] < 254)
                        return false;
                }
            }
        ...
        else if(mode == 1 || mode == 2)
        {
            ...
            for (int i = 0; i < obstacle_distance ; ++i)
            {
                ...
                {
                    cv::Scalar m = (int)map.at<float>(yTemp +  obstacle_distance- 1, xTemp+i );
                    if(m[0] < 254)
                        return false;
                }
                ...
                {
                    cv::Scalar n = (int)map.at<float>(yTemp +  obstacle_distance - 2,xTemp+i );
                    if(n[0] < 254)
                        return false;
                }
            }
        ...
        else if(mode == 2 || mode == 3)
        {
            ...
            for (int j = 0; j < obstacle_distance ; ++j)
            {
                ...
                if(is_in_map2(xTemp,yTemp,map.rows,obstacle_distance - 1,j)){
                    cv::Scalar o = (int)map.at<float>(yTemp + j,xTemp+obstacle_distance - 1  );
                    if(o[0] < 254)
                        return false;
                }
                ...
                {
                    cv::Scalar p = (int)map.at<float>(yTemp + j ,xTemp+obstacle_distance - 2  );
                    if(p[0] < 254)
                        return false;
                }
            }
        ...
        if(mode == 3 || mode == 4){
            ...
            for (int i = 0; i < obstacle_distance ; ++i)
            {
                ...
                {
                    cv::Scalar q =(int) map.at<float>(yTemp, xTemp + i );
                    if(q[0] < 254)
                        return false;
                }
                if(is_in_map2(xTemp,yTemp,map.rows,i ,1))
                {
                    cv::Scalar r = (int)map.at<float>(yTemp + 1,xTemp+i );
                    if(r[0] < 254)
                        return false;
                }
            }
...
void affiche_tree_rec_bis(Node* q_i,cv::Mat* map);
void affiche_tree_bis(Node *tree,cv::Mat* map);
...
void affiche_tree_rec_bis(Node* q_i,cv::Mat* map){
  ...
  for(i=0;i<n;i++){
    ...
    // recursive call on q_i's forest
    affiche_tree_rec_bis(q_i->forest[i],map);
...
void affiche_tree_bis(Node *tree,cv::Mat* map){
  return affiche_tree_rec_bis(tree,map);
}
