/****************************************************************************
** Meta object code from reading C++ file 'tool_manager.h'
**
** Created: Mon Sep 8 12:02:33 2014
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../src/rviz/src/rviz/tool_manager.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'tool_manager.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_rviz__ToolManager[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       4,       // signalCount

 // signals: signature, parameters, type, tag, flags
      19,   18,   18,   18, 0x05,
      35,   18,   18,   18, 0x05,
      52,   18,   18,   18, 0x05,
      71,   18,   18,   18, 0x05,

 // slots: signature, parameters, type, tag, flags
      99,   90,   18,   18, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_rviz__ToolManager[] = {
    "rviz::ToolManager\0\0configChanged()\0"
    "toolAdded(Tool*)\0toolChanged(Tool*)\0"
    "toolRemoved(Tool*)\0property\0"
    "updatePropertyVisibility(Property*)\0"
};

void rviz::ToolManager::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        ToolManager *_t = static_cast<ToolManager *>(_o);
        switch (_id) {
        case 0: _t->configChanged(); break;
        case 1: _t->toolAdded((*reinterpret_cast< Tool*(*)>(_a[1]))); break;
        case 2: _t->toolChanged((*reinterpret_cast< Tool*(*)>(_a[1]))); break;
        case 3: _t->toolRemoved((*reinterpret_cast< Tool*(*)>(_a[1]))); break;
        case 4: _t->updatePropertyVisibility((*reinterpret_cast< Property*(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData rviz::ToolManager::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject rviz::ToolManager::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_rviz__ToolManager,
      qt_meta_data_rviz__ToolManager, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &rviz::ToolManager::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *rviz::ToolManager::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *rviz::ToolManager::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_rviz__ToolManager))
        return static_cast<void*>(const_cast< ToolManager*>(this));
    return QObject::qt_metacast(_clname);
}

int rviz::ToolManager::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void rviz::ToolManager::configChanged()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}

// SIGNAL 1
void rviz::ToolManager::toolAdded(Tool * _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void rviz::ToolManager::toolChanged(Tool * _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}

// SIGNAL 3
void rviz::ToolManager::toolRemoved(Tool * _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 3, _a);
}
QT_END_MOC_NAMESPACE
